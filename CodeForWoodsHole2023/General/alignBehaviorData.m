function alignedData = alignBehaviorData(mesoTimestamps,behaviorData,behaviorTimestamps)
...
alignedData = nan(size(mesoTimestamps));
...
for i =1:length(behaviorData)
    ...
    alignedData(ind) = behaviorData(i);
end

% interpolate 
nanx = isnan(alignedData);
t    = 1:numel(alignedData);
alignedData(nanx) = interp1(t(~nanx), alignedData(~nanx), t(nanx));
