function createVidTraceMovie(alignedMovieData,alignedTraceData,movieName,varargin)
%% get plots to plot
...
% plots (hence the +1)
if numTraces == 0
    tiledlayout(numMovies,numMovies,'TileSpacing','tight');
else
...
for i =1:numMovies
    ...
    elseif strcmp(alignedMovieData.(movieFields{i}).plotType,'imagesc')
        moviePlottingData.(movieFields{i}) = imagesc(alignedMovieData.(movieFields{i}).data(:,:,1),'AlphaData', ~isnan(alignedMovieData.(movieFields{i}).data(:,:,1)));
        axis off
        % process parameters for this plot
        if isfield(alignedMovieData.(movieFields{i}),'title')
            title(alignedMovieData.(movieFields{i}).title);
...
for frameIt =2:numFrames
    ...
    for i =1:numTraces
        ...
        if ~isempty(tracePlottingData.(traceFields{i}))
            for ii = 1:length(tracePlottingData.(traceFields{i}))
